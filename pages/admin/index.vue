...
const existingAdmins = ref<Array<{ id: string; userId: string; Position: string; user: { email: string } }>>([])
...
async function loadTemplate(type: string) {
  const data = await $fetch(`/api/admin/email-templates/${type.toLowerCase()}`)
  selectedTemplate.value = type
  templateForm.value = { subject: data?.subject ?? "", html: data?.html ?? "", text: data?.text ?? "" }
}
...
async function saveTemplate() {
  await $fetch(`/api/admin/email-templates/${selectedTemplate.value.toLowerCase()}`, {
    method: 'PUT',
      ...
      <div class="flex gap-8">
        ...
        <div class="w-64 space-y-2">
          <button
            ...
            :class="[
              ...
              activeTab === 'admins' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
            ]"
          >
            Admin Users
          </button>
          <button
            ...
            :class="[
              'w-full px-4 py-2 text-left rounded-lg',
              activeTab === 'email-settings' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'
            ]"
          >
            Email Settings
          </button>
        ...
        <div class="flex-1 bg-white rounded-lg shadow p-6">
          ...
          <div v-if="activeTab === 'admins'">
            ...
            <div>
              ...
              <div class="space-y-2">
                ...
                >
                  <div>
                    <span class="font-medium">{{ admin.user.email }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="activeTab === 'email-settings'">
            ...
            <form v-if="selectedTemplate" @submit.prevent="saveTemplate">
              <input
