<template>
  <div class="text-[#d4af37] min-h-screen bg-black">
          ...
          <tbody>
            ...
            <tr class="h-9" v-for="(u, index) in nominees" :key="index">
              <td>
                ...
                <button @click="openModal(u)" class="hover:text-gray-400 underline">
                  View Nominee
                </button>
              </td>
              ...
              <td>{{ u.occupation }}</td>
              <td>{{ u.email }}</td>
              <td class="max-w-xs truncate" :title="u.description">{{ u.description }}</td>
              <td>
                <!-- Dropdown for status -->
                  ...
                  >
                    <div class="py-1 space-y-2">
                      <button
                        ...
                        @click="updateStatus(u, 'CREATED')"
                      >
                        CREATED
                      </button>
                      <button
                        class="block w-full text-center px-4 py-2 text-sm hover:bg-gray-700"
                        @click="updateStatus(u, 'CONFIRMED')"
                      >
                        CONFIRMED
                      </button>
                      <button
                        class="block w-full text-center px-4 py-2 text-sm hover:bg-gray-700"
                        @click="updateStatus(u, 'VERIFIED')"
                      >
                        VERIFIED
                      </button>
                      <button
                        class="block w-full text-center px-4 py-2 text-sm hover:bg-gray-700"
                        @click="updateStatus(u, 'APPROVED')"
                      >
                        APPROVED
                      </button>
                      <button
                        class="block w-full text-center px-4 py-2 text-sm hover:bg-gray-700"
                        @click="updateStatus(u, 'DENIED')"
                      >
                        DENIED
                      </button>
                      <button
    ...
    >
      <div class="bg-gray-800 rounded-lg w-full max-w-md px-6 py-5 shadow-xl">
        <h3 class="text-2xl font-bold mb-4 text-center">Nominee Details</h3>

        <!-- NEW: Big photo in modal -->
        <div v-if="selectedNominee.photoURL" class="flex justify-center mb-4">
          <img
        ...
        <div class="space-y-1 text-sm">
          ...
          <p class="break-words">
            <strong>Description:</strong> {{ selectedNominee.description }}
          </p>
